...
export const AddVisit: FC<AddVisitProps> = ({open, onClose, patientId}) => {
    const [request, setRequest] = useState<CreateVisitRequest>({patientId: 0, visitDate: ''})
    ...
    const handleSubmitForm = () => {
        ...
        if(request.patientId > 0 && request.visitDate !== '') {
            mutation.mutate(request, {
                onSuccess: () => snackbarContext.open('Poprawnie dodano wizytę', 'success'),
                onError: () => snackbarContext.open('Błąd przy dodawaniu wizytę', 'error')
            })
        }
    }
